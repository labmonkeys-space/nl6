...
| [`gnmic-optical/`](gnmic-optical/) | `gnmic` config consuming a simulated **Ciena Waveserver 5**'s optical telemetry, plus the degrade-endpoint loop for driving a channel across the FEC threshold on demand and telling a fibre fault from an amplifier fault. |
