...
- The engine stores the **link state** and `ifAdminStatus`; `ifOperStatus`
  is **derived** from the pair per RFC 2863 and is never stored.
- Three mutation sources: the **flap scheduler** (Poisson-distributed link
  flaps per configured scenario), the **REST control plane**
  (`POST .../oper-status`, `POST .../admin-status`) and an **SNMP `SET`** of
...
- **`InterfaceState` (`interface_state.go`)** — per-device state engine.
  ...
  `[lastChangeNs:58, admin:3, link:3]`. `ifOperStatus` is derived from
  `(admin, link)` on read, not stored, so the word is unchanged in width.
  A single atomic word guarantees a reader never sees a torn
  `(link, admin, lastChange)` tuple, and the derived `oper-status` comes from
  the same load. Reads are
...
- **Initial state (`if_counters.go`, `if_state.go`)** — each slot is seeded
  ...
  `ifOperStatus.<N>` resource rows — the latter seeding the **link** —
  overlaid by the process-wide
...
- **Flap scheduler (`flap_scheduler.go`)** — single shared min-heap
  ...
  Mirrors `trap_scheduler.go` exactly. Mutates the **link state** directly;
  each `SetLinkState` call broadcasts a `StateChange` event only when the
  derived `ifOperStatus` moves.
...
1. Atomic-update the slot: `link = DOWN`, and `lastChangeNs = elapsed` if the
   derived `ifOperStatus` moved
...
A flap on an interface whose `ifAdminStatus` is `DOWN` or `TESTING` is
**masked**: the link moves, nothing observable changes, `ifLastChange` does not
advance, and no trap, syslog or ON_CHANGE update fires. The link is still there
when the port is unshut. This is what the hardware does, and it is why
`-if-scenario 1 -if-flap-scenario aggressive` no longer produces a fleet
reporting `ifAdminStatus = down(2)` with `ifOperStatus = up(1)`.
...
**`oper-status` follows `admin-status`, asymmetrically.**
The POST goes through the engine's admin funnel (`InterfaceState.ApplyAdminStatus`).
