...
:::warning[Prefix migration: `53246` is now `5703`]
...
| `.7.{gpu}` | 7 | VRAM Total MiB (constant) | Set |
| `.8.{gpu}` | 8 | Temperature C | SetTimeSeries |
| `.9.{gpu}` | 9 | Power Draw W | SetTimeSeries |
| `.10.{gpu}` | 10 | Fan Speed % | SetTimeSeries |
| `.11.{gpu}` | 11 | SM Clock MHz | SetTimeSeries |
| `.12.{gpu}` | 12 | Memory Clock MHz | SetTimeSeries |
...
- **Attributes:** single attribute using `SnmpGpuTable` with static mapping:
  - `gpudevice.gpus` ← `SnmpGpuTable(mapping: "1:devicename:set,2:deviceuuid:set,3:serialnumber:set,4:pcibusid:set,13:driverversion:set,14:cudaversion:set,15:ecccorrectedcount:set,16:eccuncorrectedcount:set")`
  - `.17.{gpu}` (NVLink active links) has no per-GPU field in `Gpu`. The device-level `topology.nvlink_count` comes from REST.
...
- **Attributes:** single attribute using `SnmpGpuTable` with time-series mapping:
  - `gpudevice.gpus` ← `SnmpGpuTable(mapping: "5:gpuutilizationpercent:ts,6:vramusedmib:ts,7:vramtotalmib:set,8:temperaturecelsius:ts,9:powerdrawwatts:ts,10:fanspeedpercent:ts,11:smclockmhz:ts,12:memclockmhz:ts")`
...
- SSH: 14 commands (`nvidia-smi`, `nvidia-smi -q -d *`, `nvidia-smi topo -m`, `dcgmi *`,
  `show version`, `uname -a`, `hostname`, `free -h`, `lscpu`, `lspci | grep -i nvidia`).
...
| serial_number | SSH | SSH | `show version` |
