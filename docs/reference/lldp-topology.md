...
## Quick example of a fleet with a topology
...
A realistic example is the minimal 5-stage Clos from [containerlab's `min-5clos`](https://containerlab.dev/lab-examples/min-5clos/).
It has two superspines, two pods of two spines + two leaves each, and a client on a leaf in each pod.
Twelve devices, eighteen links, with a sensible device type per tier:

| Tier | Device type | Nodes | IPs | Ports used |
|------|-------------|-------|-----|------------|
| Superspine | `cisco_crs_x` (144-port core) | 2 | `10.0.0.1` to `10.0.0.2` | 1 to 4 (to the 4 spines) |
| Spine | `arista_7280r3` | 4 | `10.0.1.1` to `10.0.1.4` | 1 to 2 up (superspines), 3 to 4 down (leaves) |
| Leaf | `cisco_catalyst_9500` | 4 | `10.0.2.1` to `10.0.2.4` | 1 to 2 up (spines); 3 to a client on leaf1 / leaf3 |
| Client | `linux_server` | 2 | `10.0.3.1` to `10.0.3.2` | 1 (to its leaf) |
...
Wire the fabric.
Save it as `clos.json` and POST it:
...
A spine (`10.0.1.1`) has **4** neighbors, two superspines (ports 1 to 2) and two leaves (ports 3 to 4).
A leaf (`10.0.2.1`) has **3**, two spines and its client:
...
**Walk anchor.** LLDP lives at `1.0.8802.*`, which sorts *before* the mib-2 tree.
A walk rooted at `1.3.6.1` never reaches it.
Anchor at `1.0.8802` (and point your NMS's LLDP discovery at the LLDP root).

## Model

A link is a single **undirected** edge between two endpoints, each an `(deviceIP, ifIndex)` pair.
The graph is simulator-wide and owned by the manager.
It is the single source of truth, and each device derives its own LLDP local-system data, local-port table, and remote (neighbor) table from the edges touching it.
...
| `GET /api/v1/topology` | none | `200` `{"links":[…]}` |
| `DELETE /api/v1/topology` | `{"a":{...},"b":{...}}` | `204`, or `404` if the link is absent |
| `GET /api/v1/topology/status` | none | `200` `{"subsystem_active", "configured_links", "active_links"}` |
| `GET /api/v1/topology/graph` | none | `200` `{"nodes":[…], "edges":[…]}`, viz-ready, with live per-link state |

`active_links` counts links whose both endpoints are resolvable and operationally up.
Those are the links currently producing a neighbor row.
The difference between `configured_links` and `active_links` is your down-link count.

### Graph endpoint

`GET /api/v1/topology/graph` returns the topology as a single viz-ready object.
It joins the link graph with device identity and live per-link state, so a client renders from one fetch:
...
The graph is **edge-driven**.
Nodes are the distinct endpoints of configured links (a link-less device does not appear); a link endpoint with no live device is flagged `missing: true`.
...
**Stitching.** `lldpRemChassisId`/`lldpRemPortId` on one device are derived from the peer's own canonical sources (the `02:42:<ipv4>` chassis MAC and the peer's `ifDescr`).
These are identical to what the peer advertises in its local data, so Enlinkd matches the two half-links.
...
LLDP lives at `1.0.8802.1.1.2`, which sorts **before** the standard mib-2 tree.
A walk anchored at `1.3.6.1` will never reach it.
...
It is gated on `configured_links > 0`, so no topology means no panel.
It renders `GET /api/v1/topology/graph` with a locally-vendored sigma.js + graphology stack (no CDN; works in the `nl6sim` namespace / airgapped).
