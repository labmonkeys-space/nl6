...
| Armed count is lower than the prefix "should" match | Addresses inside a prefix with no live device are silently skipped — a typo'd prefix can half-match without any error. | Compare `participants_armed` against your expected fleet size; the planned `expect_participants` guard (issue [#391](https://github.com/labmonkeys-space/nl6/issues/391)) will make this check declarative. |
