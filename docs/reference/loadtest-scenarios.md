...
- **Flow protocols have a hard per-device rate ceiling, around 8.1–9.2 records/s.**
  ...
  Residency is `mean-flow-lifetime + half the tick interval`. The second term is the sweep delay, because expiry is noticed by a poll rather than at the instant a deadline passes. It was omitted before [nl6#462](https://github.com/labmonkeys-space/nl6/issues/462), which stated the ceiling about 5 % high and ran every paced rate a few percent low.
