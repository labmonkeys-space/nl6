...
An OID that breaks any of these is now **rejected when the catalog loads**, naming the entry and the field (nl6#539). Whether a value qualifies is decided by asking the encoder itself, so the catalog and the wire cannot disagree about what an OID is.
That covers `snmpTrapOID`, `snmpTrapEnterprise` and literal body-varbind OIDs. A **templated** varbind OID is checked only when it renders, since a `varbindOverrides` value supplied over REST can make it unencodable at fire time whatever the catalog says; such an OID is emitted as a degenerate `06 00`. That is deliberate, and it is visible: a collector will reject the message rather than record an OID nobody wrote. Before nl6#529 such an OID was silently fabricated instead, so `3.40.1` went on the wire as `.4.0.1`.
