...
- **Reverse** `<ip>` (PTR) → `ip4.mgmt.<device-name>.nl6.local`, which resolves forward to the same IP (round-trips)
...
The repo ships a ready-to-run stack at [`examples/coredns-sidecar/`](https://github.com/labmonkeys-space/nl6/tree/main/examples/coredns-sidecar):
