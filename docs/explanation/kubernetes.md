...
The simulator behaves more like a hypervisor than an application.
It owns host-level resources by name, mutates the host network stack, and has no horizontal scaling story.
Kubernetes pods, by contrast, are designed to be isolated, replicable, and free to land on any node.
...
- Network namespace name: `nl6sim` (the `nl6sim` namespace name is compiled in).
...
Each simulated device gets its own IP (`-auto-start-ip` sets the first one; `10.42.0.0/16` is the convention the examples use) on a TUN interface inside the `nl6sim` namespace on one node.
From the **host** of that node, traffic to those IPs routes via veth into the namespace and reaches the device.
From **any other pod or node** on the cluster, `10.42.0.0/16` is unknown and unreachable.

The pollers and collectors that talk to simulated devices (your NMS, Prometheus, flow / trap / syslog receivers) usually live elsewhere on the cluster network.
To make device IPs reachable from them you need one of:
...
It is a convenience, not a supported deployment, and its own README says so.
