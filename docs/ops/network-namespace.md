...
Running TUN interfaces directly in the host namespace leaks every simulated device into `systemd-networkd`, `NetworkManager`, and any firewalld / UFW / nftables policy that's installed.
At 30k interfaces the overhead is not subtle.
The `nl6sim` namespace gives the simulator a clean room with a single controlled bridge back to the host.
...
Reverse-path filtering in the kernel can silently drop packets whose source IP isn't reachable back through the receiving interface.
Two sides are relevant:

- **Simulator side.** At namespace setup the simulator sets, on the **host**, `net.ipv4.ip_forward=1`, `rp_filter=0` on `all` and on `veth-sim-host`, and `forwarding=1` on `veth-sim-host`. Nothing is set inside `nl6sim`. No user action needed.
