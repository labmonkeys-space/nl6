...
- **Pre-computed next-OID mappings** — `GETNEXT` / `WALK` without table scans.
- **Buffer pool** for SNMP reads — reduces GC pressure on sustained traffic.
- **Shared SSH / TLS keys** across all devices — avoids per-device key generation.
...
- **`htop`** during bring-up — a short CPU spike is normal as TUN interfaces come up in parallel. Steady-state load should be near-idle.
- **`ip netns exec nl6sim ip addr`** — confirm TUN interfaces exist inside the namespace. Unexpected entries in the host namespace usually mean `-no-namespace` was used.
