...
ok('closDeviceBatches: splits a tier into contiguous batches covering every IP', () => {
  ...
  // The fleet is a flat /16 management plane, so batches are plain linear:
  // each start_ip = previous start + previous count, and the union is exactly the
  // IPs one {start_ip, count} request creates (and exactly what nextHost assigns).
  ...
  // batch[2] is 200 hosts past .16.1 (linear, flat /16).
...
ok('buildClosFabric: flat /16 — linear map, device set covers endpoints, .x.0/.x.255 used (k=32)', () => {
  // The fleet is a flat /16 management plane: the index→IP map is linear base+idx
  // and .x.0/.x.255 are valid hosts the creator assigns (NOT skipped). The exact
  // device-IP set the wizard creates (every tier, chunked the way the submit path
  // does) must cover every link endpoint — the consistency the dangling-node bug
  // is about. Batches/links are linear, matching the server's nextHost under /16.
  ...
  for (const t of f.tiers) {
    for (const b of T.closDeviceBatches(t, 500)) {
      ...
      for (let i = 0; i < b.count; i++) created.add(T.intToIp(start + i));
    }
  }
  ...
  // The flat /16 plane DOES use .x.0 and .x.255 hosts (the inverse of the old
  // /24-skip behaviour) — at least one endpoint of each kind must appear.
  const lastOctets = [...endpoints].map(ip => +ip.split('.')[3]);
  assert.ok(lastOctets.includes(0), 'flat /16 should assign a .x.0 host');
