...
// TestPMTUGate_SNMPResponsesFitTheRoute covers the SNMP response path
// (nl6#489), which had no size bound at all before that change: handleGetBulk
// built maxRepetitions × repeaterCols bindings and handed the result to the
// kernel. At 10 columns × 127 repetitions that was a ~29 KB datagram in 20
// fragments.
//
// Responses are generated by the real handler and then replayed through the
// probe, the same shape as the flow case — a handler that swallows its own
// write error would otherwise hide the failure.
func TestPMTUGate_SNMPResponsesFitTheRoute(t *testing.T) {
	restoreLinkMTU(t)
	p := newPMTUProbe(t)
	if err := SetLinkMTU(p.mtu); err != nil {
		t.Fatalf("SetLinkMTU(%d): %v", p.mtu, err)
	...
	for _, c := range []struct{ cols, rep int }{
		{30, 2},   // the OpenNMS collector default
		{10, 10},  // just past the old fragmentation onset
		{10, 127}, // the old single-byte parser ceiling
		{10, 1000},
		{30, 1000},
	} {
		...
		t.Run(name, func(t *testing.T) {
			...
			resp := s.handleGetBulk(cols[0], buildGetBulkPDU(0, c.rep, cols))
			if len(resp) == 0 {
				t.Fatal("empty response; the gate would pass vacuously")
			}
			if p.wouldFragment(t, resp) {
				t.Errorf("GETBULK response is %d B (frame %d B) and needs fragmenting on a "+
					"route with MTU %d — truncation is not bounding the response",
					len(resp), len(resp)+ipv4HeaderBytes+udpHeaderBytes, p.mtu)
			}
			t.Logf("%d cols x %d reps -> %d B (frame %d B), route MTU %d",
				c.cols, c.rep, len(resp), len(resp)+ipv4HeaderBytes+udpHeaderBytes, p.mtu)
		})
	}
}

// TestPMTUGate_SNMPRefusesWhenBudgetExceedsRoute is the negative control for
// the SNMP path: with the simulator's budget raised above the route's real MTU,
// the same walk must produce a datagram the kernel refuses. A gate that cannot
// see the failure proves nothing about the pass.
func TestPMTUGate_SNMPRefusesWhenBudgetExceedsRoute(t *testing.T) {
	restoreLinkMTU(t)
	p := newPMTUProbe(t)
	if err := SetLinkMTU(p.mtu + ipv4HeaderBytes + udpHeaderBytes); err != nil {
		t.Fatalf("SetLinkMTU: %v", err)
	}
	if maxSNMPResponseSize <= p.budget() {
		t.Fatalf("budget %d did not exceed the route budget %d; the control tests nothing",
			maxSNMPResponseSize, p.budget())
	}

	s := newSizeTestServer(64)
	cols := columnsFor(10)
	resp := s.handleGetBulk(cols[0], buildGetBulkPDU(0, 127, cols))
	if !p.wouldFragment(t, resp) {
		t.Fatalf("a %d B response (frame %d B) was accepted on a route with MTU %d — the gate "+
			"cannot detect an oversized SNMP response",
			len(resp), len(resp)+ipv4HeaderBytes+udpHeaderBytes, p.mtu)
	}
}
